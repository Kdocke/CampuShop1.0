$(function() {
	...
	var pageSize = 3;
	// 获取店铺列表的URL
	var listUrl = '/campu-shop/frontend/listshops';
	// 获取店铺类别列表以及区域列表的 URL
	var searchDivUrl = '/campu-shop/frontend/listshopspageinfo';
	// 页码
	var pageNum = 1;
	...
	function getSearchDivData() {
		// 如果传入了 parentId,则取出此一类级别下面的所有二级类别
		...
		$.getJSON(url,function(data) {
			if (data.success) {
				...
				// 将拼接好的类别标签嵌入前台的 html 组件里
				$('#shoplist-search-div').html(html);
				var selectOptions = '<option value="">全部街道</option>';
				var areaList = data.areaList;
				// 遍历区域信息列表，拼接出 option 标签集
	...
	$('.shop-list').on('click', '.card', function(e) {
		...
		window.location.href = '/campu-shop/frontend/shopdetail?shopId=' + shopId;
	});
